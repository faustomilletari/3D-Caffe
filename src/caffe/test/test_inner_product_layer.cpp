...
/*********************************
 * 3D Inner Product Test
 * ******************************/

int blobShape_innerProduct[5]={2,3,4,5,6};

template <typename TypeParam>
class InnerProduct3DLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;
 protected:
  InnerProduct3DLayerTest()
      : blob_bottom_(new Blob<Dtype>(
      		std::vector<int>(blobShape_innerProduct,blobShape_innerProduct+5))),
        blob_top_(new Blob<Dtype>()) {
    // fill the values
    FillerParameter filler_param;
    UniformFiller<Dtype> filler(filler_param);
  ...
  virtual ~InnerProduct3DLayerTest() { delete blob_bottom_; delete blob_top_; }
  Blob<Dtype>* const blob_bottom_;
  Blob<Dtype>* const blob_top_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
...
TYPED_TEST_CASE(InnerProduct3DLayerTest, TestDtypesAndDevices);

TYPED_TEST(InnerProduct3DLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  InnerProductParameter* inner_product_param =
      layer_param.mutable_inner_product_param();
...
TYPED_TEST(InnerProduct3DLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
...
TYPED_TEST(InnerProduct3DLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
