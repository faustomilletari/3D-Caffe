...
TYPED_TEST(Im2colKernelTest, TestGPU) {
  ...
  for (int n = 0; n < this->blob_bottom_->num(); ++n) {
    im2col_cpu(bottom_data_cpu + this->blob_bottom_->offset(n), 2,
  ...
  for (int grid_div = 2; grid_div <= 8; grid_div++) {
    for (int n = 0; n < this->blob_bottom_->num(); ++n) {
      ...
      im2col_gpu_kernel<TypeParam, 2><<<grid_dim, CAFFE_CUDA_NUM_THREADS>>>(
...
int blobShape_im2col[5]={5, 500, 10, 10, 10};

template <typename Dtype>
class Im2col3DKernelTest : public ::testing::Test {
 protected:
  Im2col3DKernelTest()
        // big so launches > 1024 threads
      : blob_bottom_(new Blob<Dtype>(std::vector<int>(blobShape_im2col,blobShape_im2col+5))),
        blob_kernel_shape_(new Blob<int>()),
        blob_stride_(new Blob<int>()),
        blob_pad_(new Blob<int>()),
        blob_top_(new Blob<Dtype>()),
    ...
    GaussianFiller<Dtype> filler(filler_param);
    filler.Fill(this->blob_bottom_);
    vector<int> dim_blob_shape(1, 3);
    blob_kernel_shape_->Reshape(dim_blob_shape);
    blob_stride_->Reshape(dim_blob_shape);
    blob_pad_->Reshape(dim_blob_shape);
    shape_col_.resize(3);

    shape_ = blob_bottom_->shape();
    pad_ = 0;
    stride_ = 2;
    kernel_size_ = 3;
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      blob_stride_->mutable_cpu_data()[i] = stride_;
      blob_pad_->mutable_cpu_data()[i] = pad_;
      shape_col_[i] = (shape_[i+2] + 2 * pad_ - kernel_size_) / stride_ + 1;
    }
  }

  virtual ~Im2col3DKernelTest() {
    delete blob_bottom_;
    delete blob_top_;
    delete blob_top_cpu_;
    delete blob_kernel_shape_;
  ...
  std::vector<int> shape_col_;
  int pad_;
  int stride_;
  int kernel_size_;
};

TYPED_TEST_CASE(Im2col3DKernelTest, TestDtypes);

TYPED_TEST(Im2col3DKernelTest, TestGPU3D) {
  Caffe::set_mode(Caffe::GPU);

  std::vector<int> blob_shape(5,0);
  blob_shape[0] = this->shape_[0];
  blob_shape[1] = this->shape_[1] * this->kernel_size_ * this->kernel_size_ * this->kernel_size_;
  blob_shape[2] = this->shape_col_[0];
  blob_shape[3] = this->shape_col_[1];
  blob_shape[4] = this->shape_col_[2];

  // Reshape the blobs to correct size for im2col output
  this->blob_top_->Reshape(blob_shape);
  this->blob_top_cpu_->ReshapeLike(*this->blob_top_);

  const TypeParam* bottom_data_cpu = this->blob_bottom_->cpu_data();
  TypeParam* top_data_cpu = this->blob_top_cpu_->mutable_cpu_data();
  
  std::vector<int> offset_vector(5,0);
  
  // CPU Version
  for (int n = 0; n < this->blob_bottom_->shape(0); ++n) {
    offset_vector[0] = n;      
    im2col_cpu(bottom_data_cpu + this->blob_bottom_->offset(offset_vector), 3,
        this->blob_bottom_->shape().data() + 1,
        this->blob_top_cpu_->shape().data() + 1,
        this->blob_kernel_shape_->cpu_data(),
        this->blob_pad_->cpu_data(), this->blob_stride_->cpu_data(),
        top_data_cpu + this->blob_top_cpu_->offset(offset_vector)); 
  }
  
  // GPU version
  int num_kernels = this->shape_[1];
  for(int i=0; i<3; i++) {
    num_kernels *= this->shape_col_[i];
  }
  int default_grid_dim = CAFFE_GET_BLOCKS(num_kernels);
  const TypeParam* bottom_data_gpu = this->blob_bottom_->gpu_data();
  ...
  for (int grid_div = 2; grid_div <= 8; grid_div++) {
    for (int n = 0; n < this->blob_bottom_->shape(0); ++n) {
      ...
      // NOLINT_NEXT_LINE(whitespace/operators)
      offset_vector[0] = n;  
      im2col_gpu_kernel<TypeParam, 3><<<grid_dim, CAFFE_CUDA_NUM_THREADS>>>(
          num_kernels, bottom_data_gpu + this->blob_bottom_->offset(offset_vector),
          this->blob_bottom_->gpu_shape() + 1, this->blob_top_->gpu_shape() + 1,
          this->blob_kernel_shape_->gpu_data(), this->blob_pad_->gpu_data(),
          this->blob_stride_->gpu_data(),
          top_data_gpu + this->blob_top_->offset(offset_vector));
      CUDA_POST_KERNEL_CHECK;
    }
