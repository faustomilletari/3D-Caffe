 ...
 * 3D ReLu Tests
 *************************************/
int reluBlobShape[5]={2,3,4,5,6};

template <typename TypeParam>
class Neuron3DLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  Neuron3DLayerTest()
      : blob_bottom_(new Blob<Dtype>(std::vector<int>(reluBlobShape,reluBlobShape+5))),
        blob_top_(new Blob<Dtype>()) {
    Caffe::set_random_seed(1701);
    // fill the values
    FillerParameter filler_param;
  ...
  virtual ~Neuron3DLayerTest() { delete blob_bottom_; delete blob_top_; }
  Blob<Dtype>* const blob_bottom_;
  Blob<Dtype>* const blob_top_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
...
TYPED_TEST_CASE(Neuron3DLayerTest, TestDtypesAndDevices);

TYPED_TEST(Neuron3DLayerTest, TestReLU) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ReLULayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(Neuron3DLayerTest, TestReLUGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ReLULayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3, 1701, 0., 0.01);
