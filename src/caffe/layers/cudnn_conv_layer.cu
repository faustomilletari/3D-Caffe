...
void CuDNNConvolutionLayer<Dtype>::Forward_gpu(
  ...
  const Dtype* weight = this->blobs_[0]->gpu_data();
    ...
    for(int axes_idx = 0; axes_idx < this->num_spatial_axes_; axes_idx ++){
    	workspace_limit_bytes *= kernel_shape_data[axes_idx];
    }
    ...
    for (int g = 0; g < this->group_; g++) {
      ...
      CUDNN_CHECK(cudnnConvolutionForward(handle_[g],
			...
			// Bias. 
			if (this->bias_term_ && (this->num_spatial_axes_ == 2)) {
    ...
    // bias for non 2D-images
		if (this->bias_term_ && (this->num_spatial_axes_ != 2)) {
			const Dtype* bias_data = this->blobs_[1]->gpu_data();
			for (int n = 0; n < this->num_; ++n) {     
				this->forward_gpu_bias(top_data + n * this->top_dim_, bias_data);
			}
		}
